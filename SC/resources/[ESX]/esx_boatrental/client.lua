...
function OpenBikesMenu()
	...
	if data.current.value == 'bike' then
		if Config.EnablePrice then
			TriggerServerEvent("esx_boatrental:lowmoney", Config.PriceTriBike) 
	...
	if data.current.value == 'bike2' then
		if Config.EnablePrice then
			TriggerServerEvent("esx_boatrental:lowmoney", Config.PriceScorcher) 
		...
		if Config.EnableEffects then
			...
			TriggerEvent('esx:spawnVehicle', "seashark3")
		end
	...
	if data.current.value == 'bike3' then
		if Config.EnablePrice then
			TriggerServerEvent("esx_boatrental:lowmoney", Config.PriceCruiser) 
		...
		if Config.EnableEffects then
			...
			TriggerEvent('esx:spawnVehicle', "speeder2")
		end
	end
	
	if data.current.value == 'bike4' then
		if Config.EnablePrice then
			TriggerServerEvent("esx_boatrental:lowmoney", Config.PriceBmx) 
		...
		if Config.EnableEffects then
			...
			TriggerEvent('esx:spawnVehicle', "toro2")
		end
	end

	if data.current.value == 'bike5' then
		if Config.EnablePrice then
			TriggerServerEvent("esx_boatrental:lowmoney", Config.PriceBmx22) 
			TriggerEvent("chatMessage", _U('bikes'), {255,0,255}, _U('bike_pay', Config.PriceBmx22))
		end
