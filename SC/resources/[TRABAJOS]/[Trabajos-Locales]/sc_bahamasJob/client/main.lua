...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if data.current.value == 'player_dressing' then
			...
			ESX.TriggerServerCallback('esx_bahamasjob:getPlayerDressing', function(dressing)
			  local elements = {}
			  ...
			  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
				  ...
				  TriggerEvent('skinchanger:getSkin', function(skin)
					...
					ESX.TriggerServerCallback('esx_bahamasjob:getPlayerOutfit', function(clothes)
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_bithilling:sendBill', GetPlayerServerId(player), 'society_bahamas', _U('billing'), amount)
        end
...
Citizen.CreateThread(function()
    while true do
      ...
      if js_isInJob and js_Job_Name == 'bahamas' then
			  	...
			  	if distance <= 7.5 and not IsPedDeadOrDying(me, true) then	
			  		...
			  		if distance <= 1.8 and not IsPedDeadOrDying(me, true) then
			  			if not IsPedInAnyVehicle(me, false) then
			  				...
			  				if IsControlJustReleased(0, 38) and js_isInJob then
			  					ESX.UI.Menu.CloseAll()
			  		      TriggerEvent('esx_society:openBosthsMenu', 'bahamas', function(data, menu)
			  		  	    menu.close()
                  end, { wash = false })
			  				end
			  			end
...
RegisterCommand("openbahamasmenu",function()
	TriggerServerEvent("yueqweuytrquwyteruy_bahamas:eyrtqwyterqywreuytrqwuyte_bahamas")
end)

RegisterNetEvent("wtuqtwiuyqtwutqiuw_bahamas:rwytqrwuyrquywruyqt_bahamas")
AddEventHandler("wtuqtwiuyqtwutqiuw_bahamas:rwytqrwuyrquywruyqt_bahamas",function()
	OpenSocietyActionsMenu()
end)

RegisterKeyMapping('openbahamasmenu', 'Abrir Menu Bahamas', 'keyboard', 'F6' )

AddEventHandler('onResourceStart', function (resourceName)
	if(GetCurrentResourceName() == resourceName)  then
		TriggerServerEvent("ruyfwuytruyrfwyeruy_bahamas:feyiqhvisytfrd8yqfgiufiwiufiy7tfiy_bahamas")
	end
end)
...
RegisterNetEvent("feuytqwreuyfrquwyterqy_bahamas:gsfcudyafsuydruyas_bahamas")
AddEventHandler("feuytqwreuyfrquwyterqy_bahamas:gsfcudyafsuydruyas_bahamas",function(boolean, job)
	js_isInJob = boolean
	js_Job_Name = job.name
end)
