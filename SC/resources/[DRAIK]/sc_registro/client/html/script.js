...
$(function () {
    ...
    document.onkeyup = function (data) {
        if (open) {
            ...
            if (data.which == 27) {
                $.post('http://sc_registro/saveescape', JSON.stringify($(this).val()))
    ...
    $('#kaydetmedenkapat').on('click', function() {
        $.post('http://sc_registro/resetped', JSON.stringify({}))
        CloseMenu2(true)
    })
    ...
    function CloseMenu(save) {
        $.post('http://sc_registro/escape', JSON.stringify({save:save}));
    }

    function CloseMenu2(save) {
        $.post('http://sc_registro/escape2', JSON.stringify({save:save}));
    }

    $(document).on('contextmenu', function() {
        $.post('http://sc_registro/togglecursor', JSON.stringify({}));
    })
    ...
    $('#savedright').on('click', function () {
        ...
        $.post('http://sc_registro/loadsaved', JSON.stringify({
            "number": input.val()
        }));
    })
    ...
    $('#skin_string').find('input').keypress(function (e) {
        if (e.which == 13) {
            $.post('http://sc_registro/customskin', JSON.stringify($(this).val()));
        }
    })
    ...
    function inputChange(ele,inputType) {
        ...
        if (currentMenu.is($('#clothesmenu')) || $(ele).parents('.panel').hasClass('hair')) {
            ...
            $.post('http://sc_registro/updateclothes', JSON.stringify({
                "name": nameId,
    ...
    $('.tog_head').on('click', function() {
        toggleCam($(this));
        $.post('http://sc_registro/switchcam', JSON.stringify({name: 'head'}))
    })
    $('.tog_torso').on('click', function() {
        toggleCam($(this));
        $.post('http://sc_registro/switchcam', JSON.stringify({name: 'torso'}))
    })
    $('.tog_leg').on('click', function() {
        toggleCam($(this));
        $.post('http://sc_registro/switchcam', JSON.stringify({name: 'leg'}))
    })
    $('.tog_cam').on('click', function() {
        toggleCam($(this));
        $.post('http://sc_registro/switchcam', JSON.stringify({name: 'cam'}))
    })


    $('.tog_hat').on('click', function() {
        $.post('http://sc_registro/toggleclothes', JSON.stringify({name: "hats"}))
    })
    $('.tog_glasses').on('click', function() {
        $.post('http://sc_registro/toggleclothes', JSON.stringify({name: "glasses"}))
    })
    $('.tog_tops').on('click', function() {
        // dont look at this :)
        $.post('http://sc_registro/toggleclothes', JSON.stringify({name: "jackets"}))
        $.post('http://sc_registro/toggleclothes', JSON.stringify({name: "undershirts"}))
        $.post('http://sc_registro/toggleclothes', JSON.stringify({name: "torsos"}))
        $.post('http://sc_registro/toggleclothes', JSON.stringify({name: "vest"}))
        $.post('http://sc_registro/toggleclothes', JSON.stringify({name: "bags"}))
        $.post('http://sc_registro/toggleclothes', JSON.stringify({name: "neck"}))
        $.post('http://sc_registro/toggleclothes', JSON.stringify({name: "decals"}))
    })
    $('.tog_legs').on('click', function() {
        $.post('http://sc_registro/toggleclothes', JSON.stringify({name: "legs"}))
    })
    $('.tog_mask').on('click', function() {
        $.post('http://sc_registro/toggleclothes', JSON.stringify({name: "masks"}))
    })
    ...
    window.addEventListener("keydown", throttle(function (ev) {
        ...
        if (_key) {
            $.post('http://sc_registro/rotate', JSON.stringify({key: _key}))
        }
    }, 50))
    ...
    function AddPalettes() {
        ...
        $('.color_palette').on('click', function() {
            ...
            if ($(this).parents('.panel').hasClass('hair')) {
                $.post('http://sc_registro/savehaircolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value')
                }));
            }
            else {
                $.post('http://sc_registro/saveheadoverlaycolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value'),
