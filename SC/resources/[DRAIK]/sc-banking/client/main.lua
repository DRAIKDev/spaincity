  ...
  while true do
    ...
    if nearBankorATM() then
      ...
      if IsControlJustPressed(1, 38) then
        ...
        TriggerServerEvent('sc-banking:server:balance', inMenu)
      end
...
RegisterNetEvent('sc-banking:client:refreshbalance')
AddEventHandler('sc-banking:client:refreshbalance', function(balance)
  ...
  ESX.TriggerServerCallback('sc-banking:server:GetPlayerName', function(playerName)
    SendNUIMessage({
      type = "balanceHUD",
      balance = balance,
      player = playerName,
...
RegisterNUICallback('deposit', function(data)
	TriggerServerEvent('sc-banking:server:depositvb', tonumber(data.amount), inMenu)
	TriggerServerEvent('sc-banking:server:balance', inMenu)
end)

RegisterNUICallback('withdraw', function(data)
	TriggerServerEvent('sc-banking:server:withdrawvb', tonumber(data.amountw), inMenu)
	TriggerServerEvent('sc-banking:server:balance', inMenu)
end)
...
RegisterNUICallback('transfer', function(data)
	TriggerServerEvent('sc-banking:server:transfervb', data.to, data.amountt, inMenu)
	TriggerServerEvent('sc-banking:server:balance', inMenu)
end)

RegisterNetEvent('sc-banking:result')
AddEventHandler('sc-banking:result', function(type, message)
