...
RegisterServerEvent('esx_society:withdrawMoneyLogs')
AddEventHandler('esx_society:withdrawMoneyLogs', function(societyName, amount)
	local xPlayer = ESX.GetPlayerFromId(source)
	local society = GetSociety(societyName)
	amount = ESX.Math.Round(tonumber(amount))
	...
	if xPlayer.job.name == society.name then
		TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
			if amount > 0 and account.money >= amount then
				...
				xPlayer.addMoney(amount)
				xPlayer.showNotification(_U('have_withdrawn', ESX.Math.GroupDigits(amount)))		
				sendDiscord('Seguridad '..society.label..'', ''..society.label..' | Steam ID: ' .. xPlayer.name .. ' | Mensaje:  Acaba de coger: '..amount.. '$')
			else
				xPlayer.showNotification(_U('invalid_amount'))
			end
		end)
...
RegisterServerEvent('esx_society:depositMoneyLogs')
AddEventHandler('esx_society:depositMoneyLogs', function(societyName, amount)
	local xPlayer = ESX.GetPlayerFromId(source)
	local society = GetSociety(societyName)
	amount = ESX.Math.Round(tonumber(amount))
	...
	if xPlayer.job.name == society.name then
		if amount > 0 and xPlayer.getMoney() >= amount then
			TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
				...
				xPlayer.showNotification(_U('have_deposited', ESX.Math.GroupDigits(amount)))
				account.addMoney(amount)
				sendDiscord('Seguridad '..society.label..'', ''..society.label..' | Steam ID: ' .. xPlayer.name .. ' | Mensaje:  Acaba de depositar: '..amount.. '$')
			end)
		else
			xPlayer.showNotification(_U('invalid_amount'))
		end
...
webhookurl = 'https://discord.com/api/webhooks/968604551060017162/y7iD2RBZXsWrABipwSAAGk4ZKdmttfbLicohS0a25PjKbzuPu_H8Mx9Qe7yYTxlqPVcf' -- Cambiamos las XXXX por el webhooks de discord
