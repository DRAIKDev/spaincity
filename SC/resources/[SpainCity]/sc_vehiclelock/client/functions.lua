...
OpenVehicleMenu = function()
    ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(fetchedVehicles)
            ...
            for _, vehicle in ipairs(gameVehicles) do
                if DoesEntityExist(vehicle) then
                    ...
                    if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
                        table.insert(menuElements, {
                            ["label"] = GetLabelText(GetDisplayNameFromVehicleModel(vehicleProps["model"])) .. " - " .. vehicleData["plate"] .. " - " .. dstCheck .. " metros de distancia.",
        ...
        if #menuElements == 0 then
            table.insert(menuElements, {
                ["label"] = "No tienes ningun vehículo personal cerca"
            })
        end
...
ChooseVehicleAction = function(vehicleEntity, callback)
    ...
    local menuElements = {
        {
            ["label"] = (GetVehicleDoorLockStatus(vehicleEntity) == 1 and "Cerrar" or "Abrir") .. " vehículo.",
        ...
        -- },
        {
            ["label"] = (DoesBlipExist(cachedData["blips"][vehicleEntity]) and "Desactivar" or "Activar") .. " localizador GPS.",
    ...
    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "second_vehicle_menu", {
        ["title"] = "Acciones - " .. GetVehicleNumberPlateText(vehicleEntity),
        ["align"] = Config.AlignMenu,
        ["elements"] = menuElements
    }, function(menuData, menuHandle)
...
VehicleAction = function(vehicleEntity, action)
    ...
    if action == "change_lock_state" then
        if dstCheck >= Config.RangeCheck then return exports['mythic_notify']:DoHudText('error', "Estas muy lejos de tu vehículo") end
        ...
        exports['mythic_notify']:DoHudText('inform', GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicleEntity))) .."".. (vehicleLockState == 1 and " cerrado" or " abierto"))
    ...
    elseif action == "change_gps_state" then
        if DoesBlipExist(cachedData["blips"][vehicleEntity]) then
            ...
            SetBlipScale(cachedData["blips"][vehicleEntity], 0.8)
