        ...
        MySQL.Async.fetchScalar('SELECT `count` FROM truck_inventory WHERE `plate` = @plate AND `item`= @item AND `itemt`= @itemt',
            {
                ...
                if countincar >= count and count ~= nil then
...
ESX.RegisterServerCallback('esx_truck:checkifcancarry', function(source, cb, item,quantity,tipodeitem)
   local xPlayer = ESX.GetPlayerFromId(source)
    --print(item)
    --print(quantity)
    --print(tipodeitem)
    if tipodeitem == 'item_standard' then
    ...
    elseif tipodeitem == 'item_account' then
        cb(true)
        --print("1")
