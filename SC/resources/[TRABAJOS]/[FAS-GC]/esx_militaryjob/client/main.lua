...
function setUniform(uniform, playerPed)
	TriggerEvent('skinchanger:getSkin', function(skin)
		...
		if uniformObject then
			...
			if uniform == 'chaleco_normal' then
				SetPedArmour(playerPed, 100)
			end

			if uniform == 'armadura' then
				SetPedArmour(playerPed, 100)
			end

			if uniform == 'chaleco_tocho' then
				SetPedArmour(playerPed, 100)
			end
		else
...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{label = ('--Uniformes GC--'), value = 'novalido'},
		{label = ('Uniforme normal GC'), uniform = 'uniforme_normal_gc'},
		{label = ('Uniforme GC trafico'), uniform = 'uniforme_gc_trafico'},
		{label = ('Uniforme rangos altos GC'), uniform = 'uniforme_rangosaltos_gc'},
		{label = ('--Uniformes FAS--'), value = 'novalido'},
		{label = ('Uniforme normal FAS'), uniform = 'uniforme_normal_fas'},
		{label = ('--Chalecos--'), value = 'novalido'},
		{label = ('Armadura'), uniform = 'armadura'},
		{label = ('Chaleco Normal'), uniform = 'chaleco_normal'},
		{label = ('Chaleco Reflectante'), uniform = 'chaleco_reflectante'},
		{label = ('Chaleco UCO'), uniform = 'chaleco_uco'},
		{label = ('Chaleco Guardia Civil'), uniform = 'chaleco_gctocho'},
		{label = ('Chaleco Grande'), uniform = 'chaleco_tocho'}
	}
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'military')
		...
		if data.current.value == 'novalido' then
			ESX.ShowNotification('Esta opción no es valida.')
		end

		if data.current.value == 'player_dressing' then
		
			ESX.TriggerServerCallback('esx_militaryjob:getPlayerDressing', function(dressing)
			  local elements = {}
			  ...
			  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
				  ...
				  TriggerEvent('skinchanger:getSkin', function(skin)
					...
					ESX.TriggerServerCallback('esx_militaryjob:getPlayerOutfit', function(clothes)
...
function OpenArmoryMenu(station)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory', {
		...
		elseif data.current.value == 'put_stock' then
			OpenPutStocksMenu()
		elseif data.current.value == 'eliminar_armas' then
			EliminarTodasLasArmas()
...
function EliminarTodasLasArmas()
	RemoveAllPedWeapons(GetPlayerPed(-1), true)
	ESX.ShowNotification("Has depositado todas las armas que llevabas encima.")
end
...
function OpenmilitaryActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'military_actions', {
		...
		elements = {
			...
			{label = ('Menu de Refuerzos'), value = 'renfort'},
		...
		if data.current.value == 'citizen_interaction' then
			local elements = {
				...
				{label = ('Menu de Esposas'), value = 'esposas'},
				-- {label = _U('drag'), value = 'drag'}, -> Ya esta en el menu de esposas superior.
							...
							if closestPlayer ~= -1 and closestDistance <= 3.0 then
								...
								if action == 'handcuff' then
									...
									if distance <= 2.0 then
										...
										Wait(5000)
										TriggerServerEvent('esx_cnpjob:hathndcuff', GetPlayerServerId(closestPlayer),"B16")
									else
								...
								elseif action == 'quitarbridas' then
									...
									Wait(5000)
									TriggerServerEvent('esx_militaryjob:bridas1', GetPlayerServerId(closestPlayer),securityToken)
								...
								if amount == nil or amount <= 0 then
									...
									if closestPlayer == -1 or closestDistance > 3.0 then
										...
										Citizen.CreateThread(function()
											...
											TriggerServerEvent('esx_bithilling:sendBill', GetPlayerServerId(closestPlayer), 'society_military', 'military', amount)
										end)
									end
								end
							end,
		...
		elseif data.current.value == 'renfort' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'refuerzosmilitares', {
				css      = 'military',
				...
				if action == 'petite_demande' then
					local raison = 'petit'
					TriggerServerEvent('refuerzosmilitares', coords, raison)
					ExecuteCommand("me Se lleva la mano al pecho presionandolo")
	...
	ESX.TriggerServerCallback('esx_militaryjob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data.current.value then
				TriggerServerEvent('esx_militaryjob:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
	...
	ESX.TriggerServerCallback('esx_militaryjob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_bithilling:sendBill', GetPlayerServerId(player), 'society_military', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
				TriggerServerEvent('esx_bithilling:sendBill', GetPlayerServerId(player), '', _U('fine_total', data.current.fineLabel), data.current.amount)
			end
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle', {
		...
		if not data.value or length < 2 or length > 8 then
			...
			ESX.TriggerServerCallback('esx_militaryjob:getVehicleInfos', function(retrivedInfo)
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_militaryjob:getOtherPlayerData', function(playerData)
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('esx_militaryjob:buyWeapon', function(bought)
				if bought then
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'military' and ESX.PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_militaryjob:hasEnteredEntityZone', function(entity)
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'military' and IsPedOnFoot(playerPed) then
...
RegisterNetEvent('esx_militaryjob:bridas')
AddEventHandler('esx_militaryjob:bridas', function(token)
	IsHandcuffed    = not IsHandcuffed
	local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'military' then
			local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'military' then
				...
				elseif CurrentAction == 'menu_boss_actions' then
					ESX.UI.Menu.CloseAll()
					TriggerEvent('esx_society:openBosthsMenu', 'military', function(data, menu)
						menu.close()
		...
		if IsControlJustReleased(0, 167) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'military' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'military_actions') then
			if not Config.EnableESXService then
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_militaryjob:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'military')
...
-- Refuerzos Militares--
RegisterNetEvent('refuerzosmilitares:setBlip')
AddEventHandler('refuerzosmilitares:setBlip', function(coords, raison)
	if raison == 'petit' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		PlaySoundFrontend(-1, "OOB_Start", "GTAO_FM_Events_Soundset", 1)
		ESX.ShowAdvancedNotification('RADIO FAS', '~b~Peticion de refuerzos', 'Se requieren refuerzos.\n~w~Importancia: ~g~LEVE.\n \n', 'CHAR_CALL911', 8)
		Wait(1000)
