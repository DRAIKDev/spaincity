...
function deldraik(owner)
	MySQL.Async.execute('DELETE FROM datastore_data WHERE owner = @owner', {
        ['@owner'] = owner
    }) 
    for i=1, #DataStores['property'], 1 do
        if DataStores['property'][i].owner == owner then
             DataStores['property'][i] = {}
			 print("Armas de "..owner.." borrado.")
...
RegisterNetEvent('draik:dsadada')
AddEventHandler('draik:dsadada', function(hexa)
	deldraik(hexa)
end)