...
RegisterCommand("cargar",function(source, args)
	if not carryingBackInProgress then
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			ESX.TriggerServerCallback('carloslr:solicitarpago', function(can)
				TriggerServerEvent('CarryPeople:sync', closestPlayer, can , lib,lib2, anim1, anim2, 0.15, 0.24, 0.63,target,100000,0.0,49,33,1)
			end)
		else
			drawNativeNotification("Nadie cerca para cargar!")
		end
	else
