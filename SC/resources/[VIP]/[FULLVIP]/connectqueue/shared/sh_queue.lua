...
Queue.MaxPlayers = GetConvarInt("sv_maxclients", Config.MaxClients)
...
function Queue:CanJoin(src, cb)
    ...
    for _, data in ipairs(_Queue.JoinCbs) do
        ...
        while await do Citizen.Wait(1000) end
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    if not queueStarted then 
        -- prevent joining
        done(Config.Language.reinicio)
        CancelEvent()
        Queue:DebugPrint("Dropped " .. name .. ", Reiniciando servidor")
        return
    end 
    ...
    while allow == nil do Citizen.Wait(1000) end
    if not allow then return end
