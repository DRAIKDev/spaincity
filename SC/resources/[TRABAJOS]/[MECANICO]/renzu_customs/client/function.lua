...
function SetVehicleProp2(vehicle, props)
    -- https://github.com/esx-framework/es_extended/tree/v1-final COPYRIGHT
    if DoesEntityExist(vehicle) then
		local colorPrimary, colorSecondary = GetVehicleColours(vehicle)
		local pearlescentColor, wheelColor = GetVehicleExtraColours(vehicle)
...
exports('SetVehicleProp2', function(vehicle,props)
    return SetVehicleProp2(vehicle,props)
end)
        ...
        CreateThread(function()
            ...
            while #(vec - coord) <= 15 and not cancel and inmark do
                ...
                if not Config.usePopui and #(vec - coord) < 1.5 then
                    ShowFloatingHelpNotification("Presione [E] "..msg,vec)
