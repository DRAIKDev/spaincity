...
AddEventHandler('esx:setJob', function(source, job, lastJob)
  if PhoneNumbers[lastJob.name] ~= nil then
    TriggerEvent('esx_addons_gcphoneSPAIN:removeSource', lastJob.name, source)
  end

  if PhoneNumbers[job.name] ~= nil then
    TriggerEvent('esx_addons_gcphoneSPAIN:addSource', job.name, source)
  end
end)

AddEventHandler('esx_addons_gcphoneSPAIN:addSource', function(number, source)
	PhoneNumbers[number].sources[tostring(source)] = true
end)

AddEventHandler('esx_addons_gcphoneSPAIN:removeSource', function(number, source)
...
RegisterServerEvent('esx_addons_gcphoneSPAIN:startCall')
AddEventHandler('esx_addons_gcphoneSPAIN:startCall', function (number, message, coords)
  local sourcePlayer = tonumber(source)
  if PhoneNumbers[number] ~= nil then
    getPhoneNumber(source, function (phone) 
      notifyAlertSMS(number, {
...
AddEventHandler('esx:playerLoaded', function(source)
  ...
  MySQL.Async.fetchAll('SELECT * FROM users WHERE identifier = @identifier',{
    ...
    if PhoneNumbers[xPlayer.job.name] ~= nil then
      TriggerEvent('esx_addons_gcphoneSPAIN:addSource', xPlayer.job.name, source)
    end
  end)
...
AddEventHandler('esx:playerDropped', function(source)
  ...
  local xPlayer = ESX.GetPlayerFromId(source)
  if PhoneNumbers[xPlayer.job.name] ~= nil then
    TriggerEvent('esx_addons_gcphoneSPAIN:removeSource', xPlayer.job.name, source)
  end
end)
