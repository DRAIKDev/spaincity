			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					if isRimMod then
						...
						TriggerServerEvent("esx_lscustom:buyMod", 0)
		...
		if data.value == k then
			...
			if v.modType ~= nil then
				...
				elseif v.modType == 'plateIndex' then -- PLATES
					...
					elseif v.modType == 'modExtra' then -- Extras
						for id,enabled in pairs(currentMods.extras) do
							local _label = ''
							local modVal = false
							if enabled then
								_label = 'Extra #'.. id .. ' : <span style="color:tomato;">'.. _U('installed') ..'</span>'
								modVal = false
							else
								price = math.floor(vehiclePrice * v.price / 100)
								_label = 'Extra #'.. id .. ' : <span style="color:green;">$' .. price .. ' </span>'
								modVal = true
							end
							table.insert(elements, {label = _label, modType = k, modNum = id, modValue = modVal})
					end
					...
					for j = 0, modCount, 1 do
						...
						if modName ~= nil then
							...
							if j == currentMods[k] then
								...
								price = 0
								_label = GetLabelText(modName) .. ' - <span style="color:green;">$0 </span>'
							end
							table.insert(elements, {label = _label, modType = k, modNum = j})
						end
					end
