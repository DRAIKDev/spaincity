...
if Config.VapePermission == true then
	RegisterCommand("vaper", function(source, args, rawCommand)
		if IsPlayerAceAllowed(source, Config.PermissionsGroup) then
			if (tostring(args[1]) == "iniciar") then
				TriggerClientEvent("Vape:StartVaping", source, 0)
			elseif (tostring(args[1]) == "parar") then
				TriggerClientEvent("Vape:StopVaping", source, 0)
			elseif (tostring(args[1])) ~= nil then
				TriggerClientEvent("chatMessage", source, "^1 Vaper: Error: Uso incorrecto del comando debes poner /vaper (iniciar/parar)")
			end
