...
local group= nil
local CoordsRecividas = false


RegisterNetEvent('es_expectate:setgroup')
AddEventHandler('es_expectate:setgroup', function(g) group = g end)
...
local trys = 0
RegisterNetEvent('SetLocation')
AddEventHandler('SetLocation', function(coords)
    ...
    Citizen.CreateThread(function()
		...
		while (targetPlayerId == nil) do
			trys = trys + 1
			if trys >= 50 then break end
			targetPlayerId = GetPlayerFromServerId(TargetSpectate)
            Citizen.Wait(100)
        end
		trys = 0
		local playerPed = nil
        while (playerPed == nil) do
            trys = trys + 1
			if trys >= 50 then break end
			playerPed = GetPlayerPed(targetPlayerId)
            Citizen.Wait(100)
        end
		trys = 0
		local coordsX = nil
		while (coordsX == nil or #(coordsX) == 0) do
			trys = trys + 1
			if trys >= 50 then break end
			coordsX = GetEntityCoords(playerPed)
            Citizen.Wait(100)
        end
        trys = 0
        CoordsRecividas = true
        SetEntityCoords(playerPed, coords.x, coords.y, coords.z + 10)
        SetCamCoord(cam, coords.x, coords.y, coords.z)
    end)
end)
...
AddEventHandler('GetLocation', function()
    TriggerServerEvent("TomaMisCC", GetEntityCoords(GetPlayerPed(-1)))
end)

function spectate(target)
	if group == 'user' or group == nil then return end
	CoordsRecividas = false
    TriggerServerEvent('GetPlayerPos', target)
...
function getPlayersList() ESX.TriggerServerCallback('EnekoGetPlayersToSpect', function(data) return data end) end

function OpenAdminActionMenu(player)
	if group == 'user' or group == nil then return end
    ESX.TriggerServerCallback('aklsdaklsjdaklsdjaklsdjsad', function(data)
    ...
    for i=1, #data.accounts, 1 do
	  ...
	  if data.accounts["money"].money ~= nil then
		Money = data.accounts["money"].money
		else
		Money = 'No Data'
	  end
      ...
      local elements = {
        ...
        {label = 'Money: '.. data.accounts["money"].money, value = nil},
	...
	while true do
		...
		if IsControlJustReleased(1, 163) then
			if group ~= "user" then
				...
				TriggerServerEvent('es_expectate:getgroup')
			end
		end
	end
end)
...
AddEventHandler('esx_spectate:spectate', function()
	...
	ESX.TriggerServerCallback('EnekoGetPlayersToSpect', function(data)
...
Citizen.CreateThread(function()
	...
	while group == nil do
  	...
  	while true do
		...
		if InSpectatorMode and group ~= 'user' and group ~= nil and CoordsRecividas then
			...
			if targetPed ~= nil then
				...
				if IsControlJustReleased(2, 47) then OpenAdminActionMenu(targetPlayerId) end
