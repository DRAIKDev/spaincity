...
AddEventHandler("SyncPlayEmoteSource", function(emote)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isRequestAnim then
            if IsControlPressed(1, 246) and isRequestAnim then
            target, distance = GetClosestPlayer()
                if(distance ~= -1 and distance < 3) then
                    ...
                    TriggerServerEvent("ServerValidEmote", GetPlayerServerId(target), requestedemote, otheremote,securityToken)
                    isRequestAnim = false
                else
                    SimpleNotify(Config.Languages[lang]['nobodyclose'])
                end
            elseif IsControlPressed(1, 182) and isRequestAnim then
