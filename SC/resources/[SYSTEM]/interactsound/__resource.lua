shared_script '@SpainCityAC/waveshield.lua' --this line was automatically written by WaveShield

resource_manifest_version '77731fab-63ca-442c-a67b-abc70f28dfa5'

client_script 'client/main.lua'

server_script 'server/main.lua'

ui_page('client/html/index.html')

files({
    'client/html/index.html',
	'client/html/sounds/stop_vehicle.ogg',
	'client/html/sounds/stop_vehicle-2.ogg',
	'client/html/sounds/speedcamera.ogg',
	'client/html/sounds/stop_the_f_car.ogg',
	'client/html/sounds/dont_make_me.ogg',
	'client/html/sounds/clear_the_area.ogg',
	'client/html/sounds/its_over_for_you.ogg',
	'client/html/sounds/you_are_finished_dhead.ogg',
	'client/html/sounds/cant_hide_boi.ogg',
	'client/html/sounds/stop_dont_move.ogg',
	'client/html/sounds/give_yourself_up.ogg',
	'client/html/sounds/stay_right_there.ogg',
	'client/html/sounds/freeze_lspd.ogg',
	'client/html/sounds/stop_or_executed.ogg',
	'client/html/sounds/cell.ogg',
	'client/html/sounds/lock.ogg',
	'client/html/sounds/unlock.ogg',
	'client/html/sounds/SodaMachine.ogg',
	'client/html/sounds/stop_or_i_kill.ogg',
	'client/html/sounds/this_is_the_lspd.ogg',
	'client/html/sounds/move_along_people.ogg',
	'client/html/sounds/get_out_of_here_now.ogg',
	'client/html/sounds/disperse_now.ogg',
	'client/html/sounds/shoot_to_kill.ogg',
	'client/html/sounds/drop_a_missile.ogg',
	'client/html/sounds/cuff.ogg',
	'client/html/sounds/cuff2.ogg',
	'client/html/sounds/aviso1.ogg',
	'client/html/sounds/aviso2.ogg',
	'client/html/sounds/aviso3.ogg',
	'client/html/sounds/uncuff.ogg',
	'client/html/sounds/morse.ogg',
	'client/html/sounds/alarm.ogg',
	'client/html/sounds/alarm1.ogg',
	'client/html/sounds/bellding.ogg',
	'client/html/sounds/burp1.ogg',
	'client/html/sounds/burp2.ogg',
	'client/html/sounds/CashRegister.ogg',
	'client/html/sounds/DeadBoltLock.ogg',
	'client/html/sounds/DeadBoltUnlock.ogg',
	'client/html/sounds/FishingRod.ogg',
	'client/html/sounds/GoKartSeatBelt.ogg',
	'client/html/sounds/onbelt.ogg',
	'client/html/sounds/stungun.ogg',
	'client/html/sounds/moments_later.ogg'
	
})




server_script '@salty_tokenizer/init.lua'
client_script '@salty_tokenizer/init.lua'








