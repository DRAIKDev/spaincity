...
function ParkBoats()
	MySQL.Async.execute('UPDATE owned_boats SET `stored` = true WHERE `stored` = false AND type = @type', {
		['@type'] = 'boat'
...
AddEventHandler('esx_boat:takeOutVehicle', function(plate)
	...
	MySQL.Async.execute('UPDATE owned_boats SET `stored` = @stored WHERE owner = @owner AND plate = @plate', {
		['@stored'] = false,
		['@owner']  = xPlayer.identifier,
		['@plate']  = plate
	}, function(rowsChanged)
...
ESX.RegisterServerCallback('esx_boat:getGarage', function(source, cb)
	MySQL.Async.fetchAll('SELECT vehicle FROM owned_boats WHERE owner = @owner AND type = @type AND `stored` = @stored', {
