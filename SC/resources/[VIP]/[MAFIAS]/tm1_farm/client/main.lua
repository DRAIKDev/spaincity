ESX                           = nil
local areas = {
    {x = 5210.14, y = -5173.815, z = 12.34, radio = 20.5,label = "PH",id = 439,colour = 71},
}
local locales = {
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
			for i,v in pairs(plants) do
				if GetDistanceBetweenCoords(v.x, v.y, v.z, GetEntityCoords(GetPlayerPed(-1),true)) < 30.0 then
				...
				if GetDistanceBetweenCoords(v.x, v.y, v.z, GetEntityCoords(GetPlayerPed(-1),true)) <= 1.0 then
					if v.percent < 100 then
						...
						if IsControlJustPressed(1,38) then
						    errorarea(locales.getmore,3000)
						end
					else
