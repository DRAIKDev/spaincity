...
function Radio:Toggle(toggle)
	...
	if self.On and not radioConfig.AllowRadioWhenClosed then
		exports["mumble_voip"]:SetMumbleProperty("radioEnabled", toggle)
	end
...
function Radio:Add(id)
	exports["mumble_voip"]:SetRadioChannel(id)
end

-- Remove player from radio channel
function Radio:Remove()
	exports["mumble_voip"]:SetRadioChannel(0)
end
...
function SetAllowRadioWhenClosed(value)
	...
	if Radio.On and not Radio.Open and radioConfig.AllowRadioWhenClosed then
		exports["mumble_voip"]:SetMumbleProperty("radioEnabled", true)
	end
end
	...
	while true do
		...
		if Radio.Open then
			...
			if IsControlJustPressed(0, radioConfig.Controls.Toggle.Key) then
				...
				exports["mumble_voip"]:SetMumbleProperty("radioEnabled", Radio.On)
			...
			if not Radio.On then
				...
				if IsDisabledControlJustPressed(0, radioConfig.Controls.ToggleClicks.Key) then
					...
					exports["mumble_voip"]:SetMumbleProperty("micClicks", Radio.Clicks)
				end
			end
		else
...
AddEventHandler("onClientResourceStart", function(resName)
	if GetCurrentResourceName() ~= resName and "mumble_voip" ~= resName then
		return
	end
	
	exports["mumble_voip"]:SetMumbleProperty("radioClickMaxChannel", radioConfig.Frequency.Max) -- Set radio clicks enabled for all radio frequencies
	exports["mumble_voip"]:SetMumbleProperty("radioEnabled", false) -- Disable radio control
