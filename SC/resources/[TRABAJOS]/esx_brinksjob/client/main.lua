	...
	if zone == 'AnnulerMission' then
		if isInService and MissionLivraison and isJobbrinks() then
			if IsPedSittingInAnyVehicle(playerPed) and IsATruck() then
                ...
                TriggerServerEvent('esx:clientlrhackLog', "3'" .. json.encode(plaquevehicule) .. "' '" .. json.encode(plaquevehiculeactuel) .. "'")
				...
				if CollectionAction == 'collection' then
					...
					if dist <= 3.5 then
						if currentbag > 0 then
							...
							trashcollection = false
							truckdeposit = false
							ESX.ShowNotification("Recogida completada, vuelve al camion!!")
...
Citizen.CreateThread(function()
	while true do
		...
		if truckdeposit then
			...
			dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z,taillight_r.x - newposx , taillight_r.y - newposy, taillight_r.z)
			if dist <= 2.0 then
			ESX.Game.Utils.DrawText3D(vector3(taillight_r.x - newposx, taillight_r.y - newposy,taillight_r.z + 0.50), "[~g~E~s~] Echar bolsas al camion.", 1.0)
			end
		end
		...
		if trashcollection then
			...
			if dist <= 5.0 then
				if currentbag == 0 then
					ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Recoger albaran", 1.0)				
				else
					ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Recoger bolsas de dinero del carro ["..currentbag.."/"..bagsoftrash.."]", 1.0)
				end
			end
		end
...
function MissionLivraisonSelect()
    TriggerServerEvent('esx:clientlrhackLog', "MissionLivraisonSelect num")
    TriggerServerEvent('esx:clientlrhackLog', MissionRegion)
	if MissionRegion == 0 then

            TriggerServerEvent('esx:clientlrhackLog', "MissionLivraisonSelect 1")
		MissionRegion = math.random(1,2)
	end
	
	if MissionRegion == 1 then -- Los santos
            TriggerServerEvent('esx:clientlrhackLog', "MissionLivraisonSelect 2")
	...
	elseif MissionRegion == 2 then -- Blaine County
            ...
            TriggerServerEvent('esx:clientlrhackLog', "MissionLivraisonSelect 3")
		MissionNum = math.random(1, 10)
