	...
	while true do
		...
		if holdingup then
							...
							if IsControlJustReleased(1, 51) then
								...
								TriggerServerEvent('esx_vangelico_robbery:gioielrhacklli')
...
Citizen.CreateThread(function()
		--TriggerEvent('lester:createBlip', 77, 706.8829, -690.454, 31.754)
    ...
    while true do
       ...
       playerPed = GetPlayerPed(-1)
		...
		if pos then
				...
				if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 706.8829, -690.454, 31.754, true) <= 5 then
					TriggerServerEvent('lester:venlrhackdita')
                    Citizen.Wait(4000)
                    TriggerEvent('skinchanger:getSkin', function(skin)
