...
Citizen.CreateThread(function()
    ...
    for k, v in pairs(Config.Impounds) do
        addBlip(v.menu, 371, 47, Strings['impound_blip'])
    end
    for k, v in pairs(Config.Garage) do
        addBlip(v.coords, 266, 38, Strings['garage'])
    end
        ...
        while true do
            ...
            for k, v in pairs(Config.Garage) do
                ...
                if GetDistanceBetweenCoords(GetEntityCoords(me), v.coords) <= 35.0 and not IsPedDeadOrDying(me, true) then
                    DrawText3D1(v.coords-vector3(0.0, 0.0, -0.5), vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0),"Pulsa ~y~E~s~ para acceder al garaje de barcos", 0, 0.1, 0.1,255)
                    DrawMarker(35, v.coords-vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(90.0, 0.0, 90.0), vector3(3.0, 3.0, -2.0), 20, 30, 100, 150, false, false, 2, false, false, false)
                    ...
                    if GetDistanceBetweenCoords(GetEntityCoords(me), v.coords) <= 1.8 and not IsPedDeadOrDying(me, true) then
                        if not IsPedInAnyVehicle(me, false) then
                            ...
                            if IsControlJustReleased(0, 38) then
                                TriggerServerEvent('DAJDK193dsa:viewVehicles', v.spawnpoint, v.heading2, k)
                            end 
                        end
                    end
                end
        ...
        while true do
            ...
            for k, v in pairs(Config.Garage) do
                ...
                if GetDistanceBetweenCoords(GetEntityCoords(me), v.savepoint) <= 35.0 and not IsPedDeadOrDying(me, true) then
                    DrawText3D1(v.savepoint-vector3(0.0, 0.0, -0.5), vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0),"Pulsa ~y~E~s~ para guardar su barco", 0, 0.3, 0.3,150)
                    ...
                    if GetDistanceBetweenCoords(GetEntityCoords(me), v.savepoint) <= 4.8 and not IsPedDeadOrDying(me, true) then
                        if not IsPedInAnyVehicle(me, false) then
                            ...
                            if DoesEntityExist(vehicle) then
                                ...
                                if IsControlJustReleased(0, 38) then
                                    TriggerEvent('DAJDK193dsa:storeVehicle', k)   
                                end
                            end
                        end
                    end
        ...
        while true do
            ...
            for k, v in pairs(Config.Impounds) do
                ...
                if GetDistanceBetweenCoords(GetEntityCoords(me), v.menu) <= 15.0 then
                    DrawText3D1(v.menu-vector3(0.0, 0.0, -1.2), vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0),"Pulsa ~y~E~s~ para sacar su barco de confiscados", 0, 0.2, 0.2,150)
                    DrawMarker(35, v.menu-vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(3.0, 3.0, 3.0), 255, 255, 25, 50, false, false, 2, false, false, false)
                        sleep = 0
                        ...
                        if IsControlJustReleased(0, 38) then
                            ...
                            TriggerServerEvent('DAJDK193dsa:impound', v.spawnpoint)
...
RegisterNetEvent('DAJDK193dsa:storeVehicle')
AddEventHandler('DAJDK193dsa:storeVehicle', function(garage)
    ...
    TriggerServerEvent('DAJDK193dsa:storeVehicle', props, json.encode(damages), garage or 'custom')
end)

RegisterNetEvent('DAJDK193dsa:openImpound')
AddEventHandler('DAJDK193dsa:openImpound', function(sasjashja, impounded)
    ...
    function(data, menu)
        print(sasjashja)
        TriggerServerEvent('DAJDK193dsa:getImpounded', sasjashja, 300, data.current.value)
        ESX.UI.Menu.CloseAll()
    end, 
        function(data, menu)
        menu.close()
...
RegisterNetEvent('DAJDK193dsa:stored')
AddEventHandler('DAJDK193dsa:stored', function()
    DoScreenFadeOut(750)
...
RegisterNetEvent('DAJDK193dsa:spawnVehicle')
AddEventHandler('DAJDK193dsa:spawnVehicle', function(info, coords, heading)
...
RegisterNetEvent('DAJDK193dsa:view')
AddEventHandler('DAJDK193dsa:view', function(vehicles, coords, heading)
            ...
            if vehicles[i].state then
                ...
                function(data2, menu2)
                    if data2.current.value == 'y' then
                        ...
                        ESX.UI.Menu.CloseAll()
                        Wait(500) 
                        TriggerServerEvent('DAJDK193dsa:takeOut', data2.current.vehicle, coords, heading)
                        Wait(500)
                        DoScreenFadeIn(450) 
                    else
                        menu2.close()
