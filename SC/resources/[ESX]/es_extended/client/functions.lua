...
ESX.ShowNotification = function(msg, job)
	if not Config.CustomNotifications.active then
		AddTextEntry('esxNotification', msg)
		SetNotificationTextEntry('esxNotification')
		DrawNotification(false, true)
	else
		exports[Config.CustomNotifications.script]:ShowNotification(msg,job)
	end
end
...
ESX.ShowAdvancedNotification = function(title, subject, msg, icon, iconType, job)
	if not Config.CustomNotifications.active then
		...
		exports[Config.CustomNotifications.script]:ShowAdvancedNotification(title, subject, msg, icon, iconType, job)
	end
end
...
ESX.ShowInventory = function()
	...
	for k,v in ipairs(ESX.PlayerData.inventory) do
		if v.count > 0 then
			...
			table.insert(elements, {
				label = ('%s <span class="counter">「</span><span class="count">%s</span><span class="counter">」</span>'):format(v.label, v.count),
				count = v.count,
				type = 'item_standard',
				value = v.name,
				usable = v.usable,
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory_item', {
			...
			if data1.current.action == 'give' then
				...
				if #playersNearby > 0 then
					...
					ESX.TriggerServerCallback('esx:getPlayerNames2', function(returnedPlayers)
