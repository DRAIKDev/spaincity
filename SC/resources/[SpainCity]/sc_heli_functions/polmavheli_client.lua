...
local toggle_rappel = 47 -- control id to rappel out of the heli. Default: INPUT_DUCK (X)
		...
		if IsPlayerInPolmav() then
			...
			if IsHeliHighEnough(heli) then
				if IsControlJustPressed(0, toggle_helicam) then -- Toggle Helicam
					...
					TriggerServerEvent("heli:logaleboscodetermica")
...
RegisterNetEvent('heli:activocamaratermina')
AddEventHandler('heli:activocamaratermina', function()
	helicam = true
end)
...
function IsPlayerInPolmav()
	...
	local dhash = GetHashKey("polmav")
	local bhash = GetHashKey("as332")
	local chash = GetHashKey("firehawk")
	local mhash = GetHashKey("polbuzz2")
	local m2hash = GetHashKey("buzzard2")
	local m3hash = GetHashKey("pavehawk")
	local geo1hash = GetHashKey("pantera")
	local geo2hash = GetHashKey("annihilator")
	local gc1hash = GetHashKey("mh6")
	local gc1hash = GetHashKey("uh1ems")
	local gc2hash = GetHashKey("supervolito")
	local gc3hash = GetHashKey("swift")
	if IsVehicleModel(vehicle, ahash) or IsVehicleModel(vehicle, bhash) or IsVehicleModel(vehicle, chash) or IsVehicleModel(vehicle, chash) or IsVehicleModel(vehicle, dhash) or IsVehicleModel(vehicle, mhash) or IsVehicleModel(vehicle, m2hash) or IsVehicleModel(vehicle, geo1hash) or IsVehicleModel(vehicle, geo2hash) or IsVehicleModel(vehicle, gc1hash) or IsVehicleModel(vehicle, gc2hash) or IsVehicleModel(vehicle, gc3hash) or IsVehicleModel(vehicle, m3hash) then
		return true
	else
		return false
	end
