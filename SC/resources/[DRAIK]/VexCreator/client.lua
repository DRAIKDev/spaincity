...
--RegisterNUICallback('selectedspawn', function(data)
    ...
    --if location == "Ayuntamiento" then
        --DoScreenFadeOut(0)
        --Citizen.Wait(1000)
        --SetEntityCoords(PlayerPedId(),-539.07,-214.7,37.65)
        --SetEntityHeading(PlayerPedId(),207.83)
        --Citizen.Wait(2000)
        --DoScreenFadeIn(3000)
    --end
--end)
...
RegisterNUICallback('updateSkin', function(data)
    ...
    if changed(bag, data.bag) or changed(bagcolor, data.bagcolor) then
        bag = tonumber(data.bag)
        bagcolor = tonumber(data.bagcolor)
        SetPedComponentVariation(player, 5, bag, bagcolor, 2)	-- Chain
    end
