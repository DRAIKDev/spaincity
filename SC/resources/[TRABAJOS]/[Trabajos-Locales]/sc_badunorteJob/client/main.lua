...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if data.current.value == 'player_dressing' then
			...
			ESX.TriggerServerCallback('esx_badunortejob:getPlayerDressing', function(dressing)
			  local elements = {}
			  ...
			  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
				  ...
				  TriggerEvent('skinchanger:getSkin', function(skin)
					...
					ESX.TriggerServerCallback('esx_badunortejob:getPlayerOutfit', function(clothes)
...
function OpenSocietyActionsMenu()
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'badunorte_actions',
    {
      title    = _U('badunorte'),
      align    = 'top-left',
      elements = elements
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_bithilling:sendBill', GetPlayerServerId(player), 'society_badunorte', _U('billing'), amount)
        end
...
function OpenGetStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_badunortejob:getStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_badunortejob:getStockItem', itemName, count)
            end
...
function OpenPutStocksMenu()
    ...
    ESX.TriggerServerCallback('esx_badunortejob:getPlayerInventory', function(inventory)
        local elements = {}
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_badunortejob:putStockItems', itemName, count)
            end
...
Citizen.CreateThread(function()
    while true do
      ...
      if js_isInJob and js_Job_Name == 'badunorte' then
			  	...
			  	if distance <= 7.5 and not IsPedDeadOrDying(me, true) then	
			  		...
			  		if distance <= 1.8 and not IsPedDeadOrDying(me, true) then
			  			if not IsPedInAnyVehicle(me, false) then
			  				...
			  				if IsControlJustReleased(0, 38) and js_isInJob then
			  					ESX.UI.Menu.CloseAll()
			  		      TriggerEvent('esx_society:openBosthsMenu', 'badunorte', function(data, menu)
			  		  	    menu.close()
                  end, { wash = false })
			  				end
			  			end
...
RegisterCommand("openbadunortemenu",function()
	TriggerServerEvent("yueqweuytrquwyteruy_badunorte:eyrtqwyterqywreuytrqwuyte_badunorte")
end)

RegisterNetEvent("wtuqtwiuyqtwutqiuw_badunorte:rwytqrwuyrquywruyqt_badunorte")
AddEventHandler("wtuqtwiuyqtwutqiuw_badunorte:rwytqrwuyrquywruyqt_badunorte",function()
	OpenSocietyActionsMenu()
end)

RegisterKeyMapping('openbadunortemenu', 'Abrir Menu Badu Norte', 'keyboard', 'F6' )

AddEventHandler('onResourceStart', function (resourceName)
	if(GetCurrentResourceName() == resourceName)  then
		TriggerServerEvent("ruyfwuytruyrfwyeruy_badunorte:feyiqhvisytfrd8yqfgiufiwiufiy7tfiy_badunorte")
	end
end)
...
RegisterNetEvent("feuytqwreuyfrquwyterqy_badunorte:gsfcudyafsuydruyas_badunorte")
AddEventHandler("feuytqwreuyfrquwyterqy_badunorte:gsfcudyafsuydruyas_badunorte",function(boolean, job)
	js_isInJob = boolean
	js_Job_Name = job.name
end)
