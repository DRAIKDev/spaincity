...
AddEventHandler('esx_jobs:action', function(job, zone)
	...
	elseif zone.Type == "work" then
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			TriggerServerEvent('esx_jobs:starthWork', zone.Item)
		end
							...
							if playerPed == driverPed then
								...
								for i=1, #myPlate, 1 do
									if myPlate[i] == plate then
										...
										--local giveBack = ESX.Math.Round(vehicleHealth / GetVehicleEngineHealth(vehicleInCaseofDrop), 2)

										TriggerServerEvent('esx_jobs:cathution', "give_back", giveBack, 0, 0)
...
AddEventHandler('esx_jobs:hasExitedMarker', function(zone)
	TriggerServerEvent('esx_jobs:stophWork')
	hintToDisplay = "no hint to display"
	menuIsShowed = false
	hintIsShowed = false
	isInMarker = false
...
function spawnVehicle(spawnPoint, vehicle)
	hintToDisplay = 'no hint to display'
	hintIsShowed = false
	TriggerServerEvent('esx_jobs:cathution', 'take', spawnPoint, vehicle)
end
