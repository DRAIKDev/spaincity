...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if data.current.value == 'player_dressing' then
			...
			ESX.TriggerServerCallback('esx_pbcafeteriajob:getPlayerDressing', function(dressing)
			  local elements = {}
			  ...
			  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
				  ...
				  TriggerEvent('skinchanger:getSkin', function(skin)
					...
					ESX.TriggerServerCallback('esx_pbcafeteriajob:getPlayerOutfit', function(clothes)
...
function OpenSocietyActionsMenu()
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'pbcafeteria_actions',
    {
      title    = _U('pbcafeteria'),
      align    = 'top-left',
      elements = elements
    },
    function(data, menu)
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_bithilling:sendBill', GetPlayerServerId(player), 'society_pbcafeteria', _U('billing'), amount)
        end
...
function OpenGetStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_pbcafeteriajob:getStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_pbcafeteriajob:getStockItem', itemName, count)
            end
...
function OpenPutStocksMenu()
    ...
    ESX.TriggerServerCallback('esx_pbcafeteriajob:getPlayerInventory', function(inventory)
        local elements = {}
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_pbcafeteriajob:putStockItems', itemName, count)
            end
...
Citizen.CreateThread(function()
    while true do
      ...
      if js_isInJob and js_Job_Name == 'pbcafeteria' then
        ...
        for k, v in pairs(Config.Zones.Vehicles) do
			  	...
			  	if distance <= 7.5 and not IsPedDeadOrDying(me, true) then	
			  		...
			  		if distance <= 1.8 and not IsPedDeadOrDying(me, true) then
			  			if not IsPedInAnyVehicle(me, false) then
			  				ESX.ShowHelpNotification(_U('vehicle_spawner'))
			  				if IsControlJustReleased(0, 38) and js_isInJob then
			  	...
			  	if distance <= 7.5 and not IsPedDeadOrDying(me, true) then	
			  		...
			  		if distance <= 1.8 and not IsPedDeadOrDying(me, true) then
			  			if not IsPedInAnyVehicle(me, false) then
			  				...
			  				if IsControlJustReleased(0, 38) and js_isInJob then
			  					ESX.UI.Menu.CloseAll()
			  		      TriggerEvent('esx_society:openBosthsMenu', 'pbcafeteria', function(data, menu)
			  		  	    menu.close()
                  end, { wash = false })
			  				end
			  			end
...
RegisterCommand("openpbcafeteriamenu",function()
	TriggerServerEvent("yueqweuytrquwyteruy_pbcafeteria:eyrtqwyterqywreuytrqwuyte_pbcafeteria")
end)

RegisterNetEvent("wtuqtwiuyqtwutqiuw_pbcafeteria:rwytqrwuyrquywruyqt_pbcafeteria")
AddEventHandler("wtuqtwiuyqtwutqiuw_pbcafeteria:rwytqrwuyrquywruyqt_pbcafeteria",function()
	OpenSocietyActionsMenu()
end)

RegisterKeyMapping('openpbcafeteriamenu', 'Abrir Menu Cafeteria Paleto', 'keyboard', 'F6' )

AddEventHandler('onResourceStart', function (resourceName)
	if(GetCurrentResourceName() == resourceName)  then
		TriggerServerEvent("ruyfwuytruyrfwyeruy_pbcafeteria:feyiqhvisytfrd8yqfgiufiwiufiy7tfiy_pbcafeteria")
	end
end)
...
RegisterNetEvent("feuytqwreuyfrquwyterqy_pbcafeteria:gsfcudyafsuydruyas_pbcafeteria")
AddEventHandler("feuytqwreuyfrquwyterqy_pbcafeteria:gsfcudyafsuydruyas_pbcafeteria",function(boolean, job)
	js_isInJob = boolean
	js_Job_Name = job.name
end)
