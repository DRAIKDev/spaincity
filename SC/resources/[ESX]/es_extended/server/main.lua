...
Citizen.CreateThread(function()
	SetMapName('SpainCity')
	SetGameType('Spaincity by DRAIKᵈᵉᵛ')
		...
		if ESX.GetPlayerFromIdentifier(identifier) then
			...
			MySQL.Async.fetchScalar('SELECT 1 FROM users WHERE identifier = @identifier', {
				...
				if result then
					...
					{ ['@name'] = playerName, ['@identifier'] = steamhex},
...
function createESXPlayer(identifier, playerId, data)
	...
	if IsPlayerAceAllowed(playerId, "command") then
		print(('^2[INFO] ^0 Jugador: ^5%s ^0Tiene permisos via ^5Ace Perms.^7'):format(playerId))
		defaultGroup = "admin"
	else
		defaultGroup = "user"
	end
...
function loadESXPlayer(identifier, playerId, isNew)
	...
	Async.parallel(tasks, function(results)
		...
		print(('[^2INFO^0] Jugador ^5"%s" ^0Se ha conectado al servidor. ID: ^5%s^7'):format(identifier, playerId))
		TriggerEvent('JJJJJJJAAAGAY:UJHDNA892OEJQYSE',playerId)
	end)
end
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	local sourcesteam = GetPlayerName(playerId)
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			if targetXPlayer.canCarryItem(itemName, itemCount) then
				...
				sourceXPlayer.showNotification(_U('gave_item', itemCount, sourceItem.label, targetsteam))
				targetXPlayer.showNotification(_U('received_item', itemCount, sourceItem.label, sourcesteam))
				sourceXPlayer.triggerEvent('esx:giveAnimation')
            targetXPlayer.triggerEvent('esx:giveAnimation')
			else
				sourceXPlayer.showNotification(_U('ex_inv_lim', targetsteam))
			end
		else
	...
	elseif type == 'item_account' then
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			sourceXPlayer.showNotification(_U('gave_account_money', ESX.Math.GroupDigits(itemCount), Config.Accounts[itemName], targetsteam))
			targetXPlayer.showNotification(_U('received_account_money', ESX.Math.GroupDigits(itemCount), Config.Accounts[itemName], sourcesteam))
			sourceXPlayer.triggerEvent('esx:giveAnimation')
            targetXPlayer.triggerEvent('esx:giveAnimation')
		else
		...
		if sourceXPlayer.hasWeapon(itemName) then
			...
			if not targetXPlayer.hasWeapon(itemName) then
				...
				if weaponObject.ammo and itemCount > 0 then
					...
					sourceXPlayer.showNotification(_U('gave_weapon_withammo', weaponLabel, itemCount, ammoLabel, targetsteam))
					targetXPlayer.showNotification(_U('received_weapon_withammo', weaponLabel, itemCount, ammoLabel, sourcesteam))
					sourceXPlayer.triggerEvent('esx:giveAnimation')
          		  targetXPlayer.triggerEvent('esx:giveAnimation')
				else
					sourceXPlayer.showNotification(_U('gave_weapon', weaponLabel, targetsteam))
					targetXPlayer.showNotification(_U('received_weapon', weaponLabel, sourcesteam))
					sourceXPlayer.triggerEvent('esx:giveAnimation')
           			 targetXPlayer.triggerEvent('esx:giveAnimation')
				end
			else
				sourceXPlayer.showNotification(_U('gave_weapon_hasalready', targetsteam, weaponLabel))
				targetXPlayer.showNotification(_U('received_weapon_hasalready', sourcesteam, weaponLabel))
			end
		end
			...
			if targetXPlayer.hasWeapon(itemName) then
				...
				if weaponObject.ammo then
					...
					if weapon.ammo >= itemCount then
						...
						sourceXPlayer.showNotification(_U('gave_weapon_ammo', itemCount, ammoLabel, weapon.label, targetsteam))
						targetXPlayer.showNotification(_U('received_weapon_ammo', itemCount, ammoLabel, weapon.label, sourcesteam))
						sourceXPlayer.triggerEvent('esx:giveAnimation')
            			targetXPlayer.triggerEvent('esx:giveAnimation')
					end
				end
			else
				sourceXPlayer.showNotification(_U('gave_weapon_noweapon', targetsteam))
				targetXPlayer.showNotification(_U('received_weapon_noweapon', sourcesteam, weapon.label))
			end
		end
	end
end)
...
ESX.RegisterServerCallback('esx:getPlayerNames2', function(source, cb, players)
	players[source] = nil
