	...
	if DoesEntityExist(ped) and not IsEntityDead(ped) then
		if IsPedOnFoot(ped) then
			if IsPlayerAbleToVape == false then
				...
				ShowNotification("~c~Tú ~b~has comenzado a ~c~vapear.")
				Wait(Config.HelpTextLength)
				TriggerEvent("Vape:HelpFadeOut", 0)
			else
				ShowNotification("~r~Tú ya estas sujetando tu vaper.")
			end
		else
			ShowNotification("~r~No puedes hacer esto en un vehículo.")
		end
	else
		ShowNotification("~r~No puedes usar el vaper si estas incosciente.")
	end
end)
...
AddEventHandler("Vape:StopVaping", function(source)
	if IsPlayerAbleToVape == true then
		...
		ShowNotification("~c~Tú ~r~acabas de parar de ~c~vapear.")
	else
		ShowNotification("~r~No estas sujetando el vaper.")
	end
end)
AddEventHandler("Vape:Drag", function()
	if IsPlayerAbleToVape then
		...
		ShowNotification("~r~Debes de estar cogiendo tu vaper para poder hacer esto.")
	end
end)
	...
	while true do
		if IsPlayerAbleToVape then
			if DisplayText then
				local ped = GetPlayerPed(-1)
				local pedPos = GetEntityCoords(ped)
				DrawText3d(pedPos.x, pedPos.y, pedPos.z+1.20, "~c~Presiona ~b~\"E\"~c~ para vapear.")
				DrawText3d(pedPos.x, pedPos.y, pedPos.z+1.08, "~c~Presiona ~b~\"G\"~c~ para dejar de vapear.")
			end
		end
	  Wait(1)
	end
