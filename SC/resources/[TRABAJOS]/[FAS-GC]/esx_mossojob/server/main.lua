...
RegisterServerEvent('esx_policejob:hathndcuff')
AddEventHandler('esx_policejob:hathndcuff', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)
	local resource = GetCurrentResourceName()

	if xPlayer.job.name == "police" or xPlayer.job.name == "military" then
	TriggerClientEvent('esx_policejob:hathndcuff', target)
	else
...
RegisterServerEvent('esx_policejob:bridas1')
AddEventHandler('esx_policejob:bridas1', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)
	local resource = GetCurrentResourceName()
	...
	if xPlayer.job.name == "police" or xPlayer.job.name == "military" then
		TriggerClientEvent('esx_policejob:bridas', target)
	else
...
RegisterServerEvent('esx_policejob:hathndcuff2')
AddEventHandler('esx_policejob:hathndcuff2', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)
	local resource = GetCurrentResourceName()

	if xPlayer.job.name == "police" or xPlayer.job.name == "military" then
		TriggerClientEvent('esx_policejob:hathndcuff2', target)
	else
...
AddEventHandler('esx_policejob:OutVehicle', function(target)
	...
	if target == -1 then 
		TriggerEvent("8882738273-GYSYAG7IH9APNSA-SPAINCITYDRAIK",source,"Sacarpolice")
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_police', function(inventory)
		...
		if count > 0 and inventoryItem.count >= count then
			...
			if xPlayer.canCarryItem(itemName, count) then
				...
				sendDiscord('Seguridad police', 'police | Steam ID: ' .. xPlayer.name .. ' | Mensaje:  Acaba de coger, Nombre Objeto: '.. itemName .. ' - Cantidad: ' ..count)
...
AddEventHandler('esx_policejob:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_police', function(inventory)
		...
		if sourceItem.count >= count and count > 0 then
			...
			sendDiscord('Seguridad police', 'police | Steam ID: '..xPlayer.name..' | Mensaje:  Acaba de depositar, Nombre Obejeto: '..itemName..' - Cantidad: '..count)
...
ESX.RegisterServerCallback('esx_policejob:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
	...
	sendDiscord('Seguridad police', 'police | Steam ID: '..xPlayer.name..' | Mensaje:  Acaba de depositar, Nombre del Arma: '.. weaponName ..' - Cantidad: 1')
end)

ESX.RegisterServerCallback('esx_policejob:removeArmoryWeapon', function(source, cb, weaponName)
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.addWeapon(weaponName, 500)
	sendDiscord('Seguridad police', 'police | Steam ID: '..xPlayer.name..' | Mensaje:  Acaba de retirar en el deposito, Nombre del Arma: '.. weaponName ..' - Cantidad: 1')
...
AddEventHandler('esx_policejob:requestarrest', function(targetid, playerheading, playerCoords,  playerlocation)
	...
	if xPlayer.job.name == "police" or xPlayer.job.name == "military" then
		TriggerClientEvent('esx_policejob:doarreste1d', _source)
	else
...
RegisterServerEvent('refuerzospolice')
AddEventHandler('refuerzospolice', function(coords, raison)
 local _source = source
 local _raison = raison
 local xPlayer = ESX.GetPlayerFromId(_source)
 local xPlayers = ESX.GetExtendedPlayers('job', 'police')
 	for _, xPlayer in pairs(xPlayers) do
		TriggerClientEvent('refuerzospolice:setBlip', xPlayer.source, coords, _raison)
	end
