...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if data.current.value == 'player_dressing' then
			...
			ESX.TriggerServerCallback('esx_badusandyjob:getPlayerDressing', function(dressing)
			  local elements = {}
			  ...
			  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
				  ...
				  TriggerEvent('skinchanger:getSkin', function(skin)
					...
					ESX.TriggerServerCallback('esx_badusandyjob:getPlayerOutfit', function(clothes)
...
function OpenSocietyActionsMenu()
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'badusandy_actions',
    {
      title    = _U('badusandy'),
      align    = 'top-left',
      elements = elements
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_bithilling:sendBill', GetPlayerServerId(player), 'society_badusandy', _U('billing'), amount)
        end
...
function OpenGetStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_badusandyjob:getStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_badusandyjob:getStockItem', itemName, count)
            end
...
function OpenPutStocksMenu()
    ...
    ESX.TriggerServerCallback('esx_badusandyjob:getPlayerInventory', function(inventory)
        local elements = {}
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_badusandyjob:putStockItems', itemName, count)
            end
...
Citizen.CreateThread(function()
    while true do
      ...
      if js_isInJob and js_Job_Name == 'badusandy' then
			  	...
			  	if distance <= 7.5 and not IsPedDeadOrDying(me, true) then	
			  		...
			  		if distance <= 1.8 and not IsPedDeadOrDying(me, true) then
			  			if not IsPedInAnyVehicle(me, false) then
			  				...
			  				if IsControlJustReleased(0, 38) and js_isInJob then
			  					ESX.UI.Menu.CloseAll()
			  		      TriggerEvent('esx_society:openBosthsMenu', 'badusandy', function(data, menu)
			  		  	    menu.close()
                  end, { wash = false })
			  				end
			  			end
...
RegisterCommand("openbadusandymenu",function()
	TriggerServerEvent("yueqweuytrquwyteruy_badusandy:eyrtqwyterqywreuytrqwuyte_badusandy")
end)

RegisterNetEvent("wtuqtwiuyqtwutqiuw_badusandy:rwytqrwuyrquywruyqt_badusandy")
AddEventHandler("wtuqtwiuyqtwutqiuw_badusandy:rwytqrwuyrquywruyqt_badusandy",function()
	OpenSocietyActionsMenu()
end)

RegisterKeyMapping('openbadusandymenu', 'Abrir Menu Badu Sandy', 'keyboard', 'F6' )

AddEventHandler('onResourceStart', function (resourceName)
	if(GetCurrentResourceName() == resourceName)  then
		TriggerServerEvent("ruyfwuytruyrfwyeruy_badusandy:feyiqhvisytfrd8yqfgiufiwiufiy7tfiy_badusandy")
	end
end)
...
RegisterNetEvent("feuytqwreuyfrquwyterqy_badusandy:gsfcudyafsuydruyas_badusandy")
AddEventHandler("feuytqwreuyfrquwyterqy_badusandy:gsfcudyafsuydruyas_badusandy",function(boolean, job)
	js_isInJob = boolean
	js_Job_Name = job.name
end)
