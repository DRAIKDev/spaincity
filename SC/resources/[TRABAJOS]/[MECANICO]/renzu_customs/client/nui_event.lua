...
RegisterNUICallback('SetPaint', function(data, cb)
    ...
    SetVehicleModKit(vehicle,0)
	local primary,secondary = GetVehicleColours(vehicle)
	if data.type == "Color Primario" and data.option ~= 'Nacarado' then
		ClearVehicleCustomPrimaryColour(vehicle)
		SetVehicleColours(vehicle,data.index,secondary)
    elseif data.type == "Color Primario" and data.option == 'Nacarado' then
        local pearlcent,wheel = GetVehicleExtraColours(vehicle)
        SetVehicleExtraColours(vehicle,data.index,wheel)
	elseif data.type == "Color Secundario" and data.option ~= 'Nacarado' then
...
RegisterNUICallback('SetCustomColor', function(data)
    ...
    SetVehicleModKit(vehicle,0)
	local primary, secondary = GetVehicleColours(vehicle)
	if data.type == "Color Primario" then
        SetVehicleCustomPrimaryColour(vehicle, r, g, b)
	elseif data.type == "Color Secundario" then
