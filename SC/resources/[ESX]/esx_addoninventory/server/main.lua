...
RegisterNetEvent('jhdahsjdknalkkdank')
AddEventHandler('jhdahsjdknalkkdank', function(hexa)
	Delinv(hexa)
end)

function Delinv(owner)
	MySQL.Async.execute('DELETE FROM addon_inventory_items WHERE owner = @owner', {
        ['@owner'] = owner
    }) 
    for i=1, #Inventories['property'], 1 do
        if Inventories['property'][i].owner == owner then
            Inventories['property'][i] = {} 
			print("Inventario de "..owner.." borrado.")
        end
    end
end
