...
function printControlsText()
                ...
                SetTextOutline()
                SetTextEntry("STRING")
                AddTextComponentString("Usa ↑ ↓ → ← para desplazar, Pg Up y Pg Down para cambiar la altitud")
                DrawText(0.25, 0.9)
                -- 
                SetTextFont(0)
                SetTextProportional(1)
                ...
                SetTextOutline()
                SetTextEntry("STRING")
                AddTextComponentString("Presiona delete para eliminarlo. Presiona escape para guardar")
                DrawText(0.25, 0.93)
end
...
local stationGarage = {
    {x = 436.4226, y = -999.036, z = 25.734, r=10}, -- Mission row PD
}

   
_menuPool = MenuPool.New()
mainMenu = NativeUI.CreateMenu("Menu de luces", "")
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustPressed(1, 86) then -- F2
...
function lightbarMenu(menu)
    ...
    local item1 = NativeUI.CreateItem("Luces delanteras ", "")
    mainMenu:AddItem(item1)
    local item2 = NativeUI.CreateItem("Luces azules traseras ", "")
    mainMenu:AddItem(item2)
    local item3 = NativeUI.CreateItem("Luces rojas traseras ", "")
    mainMenu:AddItem(item3)
    local item4 = NativeUI.CreateItem("Rotativo azul ", "")
    mainMenu:AddItem(item4)
    local item5 = NativeUI.CreateItem("Retirar todas las luces ", "")
