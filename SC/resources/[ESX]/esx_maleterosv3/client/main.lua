  ...
  --		end
			  ...
			  if quantity > 0 and quantity <= tonumber(data3.current.count) and vehFront > 0  then
				...
				--  VehicleMaxSpeed(closecar,totalweight,Config.VehicleLimit[GetVehicleClass(closecar)])
				  ...
				  ESX.ShowNotification('Has superado el limite de peso ~r~ '..MaxVh..' Kg')
				end
			  else
		  ...
		  elseif data.current.type == 'cancel' then
			...
			local tipodeitem = data.current.type
  
			  if quantity > 0 and quantity <= tonumber(data.current.count) and vehFront > 0 then
			  print("sdjakdj")
			  			ESX.TriggerServerCallback('esx_truck:checkifcancarry',function(valid)
								...
								if valid then
									...
									ESX.ShowNotification('~r~No puedes llevar eso')
			 					end
						end, item,quantity,tipodeitem)
			else
				  ESX.ShowNotification('~r~ Cantidad invalida')
			end
