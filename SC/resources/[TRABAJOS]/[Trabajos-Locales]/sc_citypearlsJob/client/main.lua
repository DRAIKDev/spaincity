...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if data.current.value == 'player_dressing' then
			...
			ESX.TriggerServerCallback('esx_citypearlsjob:getPlayerDressing', function(dressing)
			  local elements = {}
			  ...
			  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
				  ...
				  TriggerEvent('skinchanger:getSkin', function(skin)
					...
					ESX.TriggerServerCallback('esx_citypearlsjob:getPlayerOutfit', function(clothes)
...
function OpenSocietyActionsMenu()
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'citypearls_actions',
    {
      title    = _U('citypearls'),
      align    = 'top-left',
      elements = elements
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_bithilling:sendBill', GetPlayerServerId(player), 'society_citypearls', _U('billing'), amount)
        end
...
function OpenGetStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_citypearlsjob:getStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_citypearlsjob:getStockItem', itemName, count)
            end
...
function OpenPutStocksMenu()
    ...
    ESX.TriggerServerCallback('esx_citypearlsjob:getPlayerInventory', function(inventory)
        local elements = {}
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_citypearlsjob:putStockItems', itemName, count)
            end
...
Citizen.CreateThread(function()
    while true do
      ...
      if js_isInJob and js_Job_Name == 'citypearls' then
			  	...
			  	if distance <= 7.5 and not IsPedDeadOrDying(me, true) then	
			  		...
			  		if distance <= 1.8 and not IsPedDeadOrDying(me, true) then
			  			if not IsPedInAnyVehicle(me, false) then
			  				...
			  				if IsControlJustReleased(0, 38) and js_isInJob then
			  					ESX.UI.Menu.CloseAll()
			  		      TriggerEvent('esx_society:openBosthsMenu', 'citypearls', function(data, menu)
			  		  	    menu.close()
                  end, { wash = false })
			  				end
			  			end
...
RegisterCommand("opencitypearlsmenu",function()
	TriggerServerEvent("yueqweuytrquwyteruy_citypearls:eyrtqwyterqywreuytrqwuyte_citypearls")
end)

RegisterNetEvent("wtuqtwiuyqtwutqiuw_citypearls:rwytqrwuyrquywruyqt_citypearls")
AddEventHandler("wtuqtwiuyqtwutqiuw_citypearls:rwytqrwuyrquywruyqt_citypearls",function()
	OpenSocietyActionsMenu()
end)

RegisterKeyMapping('opencitypearlsmenu', 'Abrir Menu Pearls', 'keyboard', 'F6' )

AddEventHandler('onResourceStart', function (resourceName)
	if(GetCurrentResourceName() == resourceName)  then
		TriggerServerEvent("ruyfwuytruyrfwyeruy_citypearls:feyiqhvisytfrd8yqfgiufiwiufiy7tfiy_citypearls")
	end
end)
...
RegisterNetEvent("feuytqwreuyfrquwyterqy_citypearls:gsfcudyafsuydruyas_citypearls")
AddEventHandler("feuytqwreuyfrquwyterqy_citypearls:gsfcudyafsuydruyas_citypearls",function(boolean, job)
	js_isInJob = boolean
	js_Job_Name = job.name
end)
