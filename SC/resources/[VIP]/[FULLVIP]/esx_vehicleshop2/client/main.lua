	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop',
	{
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_truckshop:chk_vip', function(isVip)
				if isVip then
	...
	while true do
		...
		for k,v in pairs(Config.Zones) do
			if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
				...
				-- ESX.ShowFloatingHelpNotification("Presiona ~r~[E]~s~ para acceder.",v.Pos.x, v.Pos.y, v.Pos.z)
			end
		end
...
-- Key controls
Citizen.CreateThread(function()
	SpawnNPC('cs_milton', Config.Coords.Ped.Pos, Config.Coords.Ped.Heading)
	while true do
		Citizen.Wait(10)
