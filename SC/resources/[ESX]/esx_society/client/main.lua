...
AddEventHandler('esx_addonaccount:setMoney', function(society, money)
	if ESX.PlayerData.job and (ESX.PlayerData.job.grade_name == 'boss' or ESX.PlayerData.job.grade_name == 'viceboss') and 'society_' .. ESX.PlayerData.job.name == society then
		UpdateSocietyMoneyHUDElement(money)
	end
end)
...
function OpenBossMenuLogs(society, close, options)
	options = options or {}
	local elements = {}
				...
				if data.current.value == 'withdraw_society_money' then
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'withdraw_society_money_amount_' .. society, {
						...
						if amount == nil then
							...
							menu2.close()
							TriggerServerEvent('esx_society:withdrawMoneyLogs', society, amount)
						end
					end, function(data2, menu2)
						menu2.close()
					end)
				elseif data.current.value == 'deposit_money' then
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'deposit_money_amount_' .. society, {
						...
						if amount == nil then
							...
							menu2.close()
							TriggerServerEvent('esx_society:depositMoneyLogs', society, amount)
						end
					end, function(data2, menu2)
						menu2.close()
					end)
...
AddEventHandler('esx_society:openBosthsMenu', function(society, close, options)

	OpenBossMenu(society, close, options)
end)

AddEventHandler('esx_society:openBosthsMenuLogs', function(society, close, options)
	OpenBossMenuLogs(society, close, options)
end)
