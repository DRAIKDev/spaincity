								...
								ESX.TriggerServerCallback('esx_vehicleshop:buyVehicleSociety', function (hasEnoughMoney)
									...
									if hasEnoughMoney then
										...
										ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function (vehicle)
											...
											TriggerServerEvent('esx_vehicleshop:setVehicleOwnedthSociety', playerData.job.name, vehicleProps)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',
	{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						TriggerServerEvent('esx_bithilling:sendBill', GetPlayerServerId(closestPlayer), 'society_cardealer', _U('car_dealer'), tonumber(data2.value))
					end
				end
			end, function (data2, menu2)
				menu2.close()
		...
		elseif action == 'set_vehicle_owner_sell' then
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				if Config.EnableOwnedVehicles then
					TriggerServerEvent('esx_vehicleshop:setVehicleOwnedthPlayerId', GetPlayerServerId(closestPlayer), vehicleProps)
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				ESX.TriggerServerCallback('esx:getOtherPlayerData', function (xPlayer)
					...
					if Config.EnableSocietyOwnedVehicles then
						TriggerServerEvent('esx_vehicleshop:setVehicleOwnedthSociety', xPlayer.job.name, vehicleProps)
						ESX.ShowNotification(_U('vehicle_set_owned', vehicleProps.plate, GetPlayerName(closestPlayer)))
					else
						ESX.ShowNotification(_U('vehicle_sold_to', vehicleProps.plate, GetPlayerName(closestPlayer)))
					end
...
function OpenBossActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		...
		if data.current.value == 'boss_actions' then
			TriggerEvent('esx_society:openBosthsMenu', 'cardealer', function(data2, menu2)
				menu2.close()
			end)
	...
	while true do
		...
		for k,v in pairs(Config.Zones) do
			if(GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < v.Size.x) then
				ESX.ShowFloatingHelpNotification("Presiona ~r~[E]~s~ para ver catalogo.", Config.Coords.Ped.Text)
				isInMarker  = true
				currentZone = k
			end
		end
...
-- Key controls
Citizen.CreateThread(function()
	SpawnNPC('cs_bankman', Config.Coords.Ped.Pos, Config.Coords.Ped.Heading)
	while true do
		Citizen.Wait(10)
		...
		if CurrentAction == nil then
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				elseif CurrentAction == 'resell_vehicle' then
					...
					for i = 1, #Config.modelosbanned,1 do
						Citizen.Trace(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)))
						Citizen.Trace(Config.modelosbanned[i])
						if GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)) == Config.modelosbanned[i] then 
							isbanned = true
							break
						end
					end
...
function SpawnNPC(modelo, x, y, z, h)
	...
	TaskPlayAnim(crearNPC,"mini@strip_club@idles@bouncer@base","base", 8.0, 0.0, -1, 1, 0, 0, 0, 0)
end
