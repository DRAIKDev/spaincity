...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if data.current.value == 'player_dressing' then
			...
			ESX.TriggerServerCallback('esx_baducentraljob:getPlayerDressing', function(dressing)
			  local elements = {}
			  ...
			  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
				  ...
				  TriggerEvent('skinchanger:getSkin', function(skin)
					...
					ESX.TriggerServerCallback('esx_baducentraljob:getPlayerOutfit', function(clothes)
...
function OpenSocietyActionsMenu()
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'baducentral_actions',
    {
      title    = _U('baducentral'),
      align    = 'top-left',
      elements = elements
    },
    function(data, menu)
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_bithilling:sendBill', GetPlayerServerId(player), 'society_baducentral', _U('billing'), amount)
        end
...
function OpenGetStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_baducentraljob:getStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_baducentraljob:getStockItem', itemName, count)
            end
...
function OpenPutStocksMenu()
    ...
    ESX.TriggerServerCallback('esx_baducentraljob:getPlayerInventory', function(inventory)
        local elements = {}
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_baducentraljob:putStockItems', itemName, count)
            end
...
Citizen.CreateThread(function()
    while true do
      ...
      if js_isInJob and js_Job_Name == 'baducentral' then
			  	...
			  	if distance <= 7.5 and not IsPedDeadOrDying(me, true) then	
			  		...
			  		if distance <= 1.8 and not IsPedDeadOrDying(me, true) then
			  			if not IsPedInAnyVehicle(me, false) then
			  				...
			  				if IsControlJustReleased(0, 38) and js_isInJob then
			  					ESX.UI.Menu.CloseAll()
			  		      TriggerEvent('esx_society:openBosthsMenu', 'baducentral', function(data, menu)
			  		  	    menu.close()
                  end, { wash = false })
			  				end
			  			end
...
RegisterCommand("openbaducentralmenu",function()
	TriggerServerEvent("yueqweuytrquwyteruy_baducentral:eyrtqwyterqywreuytrqwuyte_baducentral")
end)

RegisterNetEvent("wtuqtwiuyqtwutqiuw_baducentral:rwytqrwuyrquywruyqt_baducentral")
AddEventHandler("wtuqtwiuyqtwutqiuw_baducentral:rwytqrwuyrquywruyqt_baducentral",function()
	OpenSocietyActionsMenu()
end)

RegisterKeyMapping('openbaducentralmenu', 'Abrir Menu Badu Central', 'keyboard', 'F6' )

AddEventHandler('onResourceStart', function (resourceName)
	if(GetCurrentResourceName() == resourceName)  then
		TriggerServerEvent("ruyfwuytruyrfwyeruy_baducentral:feyiqhvisytfrd8yqfgiufiwiufiy7tfiy_baducentral")
	end
end)
...
RegisterNetEvent("feuytqwreuyfrquwyterqy_baducentral:gsfcudyafsuydruyas_baducentral")
AddEventHandler("feuytqwreuyfrquwyterqy_baducentral:gsfcudyafsuydruyas_baducentral",function(boolean, job)
	js_isInJob = boolean
	js_Job_Name = job.name
end)
